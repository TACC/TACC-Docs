...
**Horizon SUs billed = (# nodes) x (job duration in wall clock hours) x (charge rate per node-hour)**
...
!!! note
	TACC does not implement node-sharing on any compute resource. Each Horizon node can be assigned to only one user at a time; hence a complete node is dedicated to a user's job and accrues wall-clock time for all the node's cores whether or not all cores are used.
