...
This page documents how to run the PyTorch multi-GPU sanity test inside an Apptainer container on TACC systems.
...
4.  Bind `$SCRATCH`
5.  Run `torchrun` inside the container
...
1. Run training code/sanity test on GPU
    ...
    apptainer exec --nv pytorch.sif   torchrun --nproc_per_node=4   examples/distributed/ddp-tutorial-series/multigpu_torchrun.py 50 10
