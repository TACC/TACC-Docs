# PyTorch at TACC or Overview
*Last update: April 21, 2026*

Running PyTorch on TACC systems
...
Run a **multi-GPU PyTorch sanity test** on TACC systems.

You will:

* Request a GPU node (`idev`)
* Set up an environment
* Run a distributed test using `torchrun`
...
* Work inside an `idev` session
* Use `$SCRATCH` for all files and environments
* Do not use `$HOME` or `$WORK`
...
```bash
cd $SCRATCH
idev -p <gpu-queue> -t 02:00:00
```

---

## Core Workflow (All Methods)
...
Move to batch jobs (`sbatch`) after validation.
...
### Table 1. `idev` invocations for accessing GPU nodes
...
1.  Install Conda and perform all computations in the `$SCRATCH` file system, and within an `idev` session..
...
1.  Move to the scratch directory and Launch an idev session
...
1.  Launch an interactive session on a compute node with TACC's `idev` utility. Consult the table above for the appropriate idev invocation per resource. Each command requests 1 GPU node for two hours in the appropriate GPU queue. The following command is run on Frontera.
...
After validating workflows with `idev`, move your production workloads to Slurm batch jobs.
